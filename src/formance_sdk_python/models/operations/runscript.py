...
from formance_sdk_python.models.ledger import (
    script as ledger_script,
    scriptresponse as ledger_scriptresponse,
)
...
class RunScriptResponseTypedDict(TypedDict):
    ...
    script_response: NotRequired[ledger_scriptresponse.ScriptResponseTypedDict]
...
class RunScriptResponse(BaseModel):
    ...
    script_response: Optional[ledger_scriptresponse.ScriptResponse] = None
    r"""On success, it will return a 200 status code, and the resulting transaction under the `transaction` field.
    ...
    @model_serializer(mode="wrap")
    def serialize_model(self, handler):
        optional_fields = set(["ScriptResponse"])
        serialized = handler(self)
        m = {}
