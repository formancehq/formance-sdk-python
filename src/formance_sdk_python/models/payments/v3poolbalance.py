"""Code generated by Speakeasy (https://speakeasy.com). DO NOT EDIT."""

from __future__ import annotations
from formance_sdk_python.types import (
    BaseModel,
    Nullable,
    OptionalNullable,
    UNSET,
    UNSET_SENTINEL,
)
from formance_sdk_python.utils import validate_int
import pydantic
from pydantic import model_serializer
from pydantic.functional_validators import BeforeValidator
from typing import List
from typing_extensions import Annotated, NotRequired, TypedDict


class V3PoolBalanceTypedDict(TypedDict):
    amount: int
    asset: str
    related_accounts: NotRequired[Nullable[List[str]]]


class V3PoolBalance(BaseModel):
    amount: Annotated[int, BeforeValidator(validate_int)]

    asset: str

    related_accounts: Annotated[
        OptionalNullable[List[str]], pydantic.Field(alias="relatedAccounts")
    ] = UNSET

    @model_serializer(mode="wrap")
    def serialize_model(self, handler):
        optional_fields = set(["relatedAccounts"])
        nullable_fields = set(["relatedAccounts"])
        serialized = handler(self)
        m = {}

        for n, f in type(self).model_fields.items():
            k = f.alias or n
            val = serialized.get(k, serialized.get(n))
            is_nullable_and_explicitly_set = (
                k in nullable_fields
                and (self.__pydantic_fields_set__.intersection({n}))  # pylint: disable=no-member
            )

            if val != UNSET_SENTINEL:
                if (
                    val is not None
                    or k not in optional_fields
                    or is_nullable_and_explicitly_set
                ):
                    m[k] = val

        return m


try:
    V3PoolBalance.model_rebuild()
except NameError:
    pass
